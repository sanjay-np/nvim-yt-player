...
function M.open_picker()
    ...
    for i, item in ipairs(history) do
        ...
        table.insert(lines, string.format("    ⏱ %s  •  %s", fmt_duration(item.duration), relative_time(item.timestamp)))
        table.insert(lines, "")
    end
    if #lines > 2 then table.remove(lines) end -- remove trailing spacer
    ...
    local function get_current_item()
        ...
        if r < 3 or #history == 0 then return nil end
    ...
    local function queue_item()
        ...
        if item then
            ...
            if not mpv.is_running() then
                ...
                vim.notify("YT Control: Queued → " .. item.title, vim.log.levels.INFO)
            end
        end
    end
